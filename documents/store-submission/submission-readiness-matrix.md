...
| Signed Android release artifact | Red | Mobile engineer / Release manager | Signed `.aab` built from release config | Release config hardened, but no actual `.aab` or keystore in repo | Yes | Follow `documents/release-evidence/android-signed-release-runbook.md`, then record the result in `documents/release-evidence/signed-build-evidence.md` |
