...
function FuelManagement() {
  ...
  async function loadFuelLogs() {
    ...
    try {
      ...
      setError(
        getErrorMessage(
          error,
          'Unable to load fuel logs right now. Check that the backend is running.',
        ),
      )
    } finally {
      setIsLoading(false)
  ...
  */
  ...
  async function handleSubmit(
    ...
    if (
      ...
      setError(
        'Vehicle and fuel card are required.',
      )
    ...
    try {
      if (
        ...
        setSuccessMessage(
          'Fuel log saved to the backend.',
        )
      } else {
        const response =
          await api.put<FuelLog>(
            `/fuel-logs/${editingLogId}`,
            payload,
          )
  ...
  */
  ...
  async function handleDelete(
    ...
    const confirmed =
      window.confirm(
        `Delete the fuel log for ${log.vehicle} on ${log.date}?`,
      )
  ...
  return (
    <div className="fuel-management">
      ...
      <div className="card fuel-log-card">
        ...
        {isLoading ? null : safeLogs.length ===
          0 ? (
          <p>
            No fuel logs
            recorded yet.
