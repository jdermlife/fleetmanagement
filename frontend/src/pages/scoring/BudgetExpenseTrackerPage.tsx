...
export default function BudgetExpenseTrackerPage() {
  ...
  const step8Accounts = useMemo(() => NET_WORTH_STATEMENT_ENTRIES.filter((entry) =>
    !entry.autoGenerated
    && ['assets', 'liabilities', 'monthly-income', 'monthly-expenses'].includes(entry.section)), []);
  const journalTotals = useMemo(() => journalEntries.reduce((totals, entry) => ({
    debit: totals.debit + toSafeNumber(entry.debitAmount),
    credit: totals.credit + toSafeNumber(entry.creditAmount),
  }), { debit: 0, credit: 0 }), [journalEntries]);
  const journalDifference = journalTotals.debit - journalTotals.credit;

  const openBuildProfileStep8 = () => {
    const query = selectedApplicationNo
      ? `?applicationNo=${encodeURIComponent(selectedApplicationNo)}&step=8`
      : '?step=8';
          ...
          <article className="psychometric-panel budget-expense-voice-form">
            ...
            {step === 3 ? (
              <div className="budget-workflow-step-block">
                ...
                <section className="budget-journal-entry-panel" aria-labelledby="budget-journal-entry-title">
                  <div className="psychometric-panel-header">
                    <div>
                      <span className="psychometric-panel-kicker">Step 8 Accounts</span>
                      <h3 id="budget-journal-entry-title">Debit and Credit Entries</h3>
                    </div>
                    <button type="button" className="budget-dashboard-category-reset" onClick={openBuildProfileStep8}>
                      Open Build Profile Step 8
                    </button>
                  </div>
                  <p className="psychometric-section-note">
