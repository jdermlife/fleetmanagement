  ...
  WORKFLOW_STEPS.forEach(({ id }) => {
    if (id === 8) {
      const completedFields = [profile.values.asOfDate, profile.values.financialGoal, profile.values.wealthCurrency || 'PHP'].filter((value) => value?.trim()).length
        const hasMonthlyIncomeEntry = NET_WORTH_STATEMENT_ENTRIES
          .some((entry) => entry.section === 'monthly-income' && profile.values[entry.id]?.trim())
        const hasMonthlyExpenseEntry = NET_WORTH_STATEMENT_ENTRIES
          .some((entry) => entry.section === 'monthly-expenses' && profile.values[entry.id]?.trim())
        result[id] = Math.round(((completedFields + (hasMonthlyIncomeEntry && hasMonthlyExpenseEntry ? 1 : 0)) / 4) * 100)
    }
    else if (id === 9) {
