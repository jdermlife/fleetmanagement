...
export default function BuildProfilePage() {
  ...
  const stepCompletion = useMemo(() => {
    ...
    WORKFLOW_STEPS.forEach(({ id }) => {
      if (id === 8) {
        ...
        const targetEntries = NET_WORTH_STATEMENT_ENTRIES.filter((entry) => profile.values[`wealthTarget.${entry.id}`] !== undefined)
  ...
  const renderCurrentStep = () => {
    ...
    if (profile.step === 8) {
      ...
      return <div className="build-profile-step-content build-profile-step-eight">
        ...
        <details className="build-profile-detail-section build-profile-net-worth-statement build-profile-income-expense-statement">
          <summary>Current Personal Income and Expenses with Goals and Protection Here</summary>
    ...
    if (profile.step === 9) {
      ...
      const actualAiAdvisories = computeAiAdvisories({
        score: actualScore,
        amounts: actualAmounts,
        labels: Object.fromEntries(NET_WORTH_STATEMENT_ENTRIES.map((entry) => [entry.id, entry.label])),
        currency,
      ...
      const renderActualColumn = (title: string, rows: typeof actualRows, tone: string) => <section className={`build-profile-income-expense-column build-profile-income-expense-${tone}`}>
        ...
        <div className="build-profile-income-expense-lines">
          {[...new Set(rows.map((entry) => entry.category))].map((category) => <div key={category} className="build-profile-income-expense-group">
            ...
            {rows.filter((entry) => entry.category === category).map((entry) => <label key={entry.id} className="build-profile-detailed-net-worth-line">
              <span>{entry.label}</span>
              <NumericFormat value={profile.values[`wealthActual.${entry.id}`] ?? ''} valueIsNumericString thousandSeparator="," decimalScale={2} fixedDecimalScale inputMode="decimal" allowNegative={false} placeholder="" aria-label={`${entry.label} actual statement amount`} onValueChange={({ value }) => updateValue(`wealthActual.${entry.id}`, value)} />
            </label>)}
          </div>)}
        </div>
      ...
      const saveActualSetup = () => {
        ...
        setSaveMessage('Actual financial statement saved. Continue with Step 10.')
      }
      ...
      return <div className="build-profile-step-content build-profile-step-nine">
        ...
        <details className="build-profile-detail-section build-profile-net-worth-statement build-profile-detailed-net-worth build-profile-actual-net-worth">
          <summary><strong>Net Worth Aspiration</strong></summary>
          <div className="build-profile-net-worth-meta"><span>Aspired Net Worth automatically captures existing assets and liabilities from Step 8. Make changes to fit your goal.</span><label>Actual As Of <input aria-label="Actual statement as of date" type="date" value={profile.values.wealthActualAsOfDate ?? ''} onChange={(event) => updateValue('wealthActualAsOfDate', event.target.value)} /></label></div>
          <div className="build-profile-net-worth-columns">
            {(['assets', 'liabilities'] as const).map((section) => {
        ...
        <details className="build-profile-detail-section build-profile-net-worth-statement build-profile-income-expense-statement build-profile-actual-income-expense">
          <summary> Personal Income and Expense Objective</summary>
          <div className="build-profile-net-worth-meta"><span>Actual Income, Expenses, Goals and Protection</span><strong>Actual values</strong></div>
          <div className="build-profile-net-worth-columns build-profile-income-expense-columns">
        ...
        <details className="build-profile-detail-section build-profile-statement-filters">
          ...
          <div className="build-profile-form-grid">
            <label>Statement Section<select aria-label="Actual filter by statement section" value={targetSectionFilter} onChange={(event) => { setTargetSectionFilter(event.target.value as 'all' | StatementSection); setTargetCategoryFilter('all') }}><option value="all">All Sections</option>{STEP1_SECTION_ORDER.map((section) => <option key={section} value={section}>{STEP1_SECTION_SHORT_LABELS[section]}</option>)}</select></label>
            <label>Account Group<select aria-label="Actual filter by account group" value={targetCategoryFilter} onChange={(event) => setTargetCategoryFilter(event.target.value)}><option value="all">All Account Groups</option>{categories.map((category) => <option key={category} value={category}>{category}</option>)}</select></label>
            <label>Line Item<input type="search" aria-label="Actual filter by line item" placeholder="Search line item" value={targetLineSearch} onChange={(event) => setTargetLineSearch(event.target.value)} /></label>
          ...
          {STEP1_SECTION_ORDER.map((section) => {
            ...
            return <details key={section} className="build-profile-detail-section" open={targetSectionFilter === section || (targetSectionFilter === 'all' && section === 'assets')}>
              ...
              {entries.length === 0 ? <p className="psychometric-section-note">No matching accounts.</p> : <div className="build-profile-wealth-entry-grid">
                {entries.map((entry) => <article key={entry.id} className={entry.autoGenerated ? 'build-profile-ai-advisory-card' : undefined} tabIndex={entry.autoGenerated ? 0 : undefined}>
                  <span>{entry.label}</span><small>{entry.category}</small>
                  {entry.autoGenerated ? <><output>{actualAiValues[entry.id] ?? 'Auto-calculated'}</output><aside className="build-profile-ai-advisory" role="note" aria-label={`${entry.label} actual AI advisory`}><strong>FILSCORE AI Analysis</strong><ul>{actualAiAdvisories[entry.id]?.analysis.map((item) => <li key={item}>{item}</li>)}</ul><strong>AI Recommendation</strong><p>{actualAiAdvisories[entry.id]?.recommendation}</p></aside></> : <NumericFormat aria-invalid={!profile.values[`wealthActual.${entry.id}`]?.trim()} value={profile.values[`wealthActual.${entry.id}`] ?? ''} valueIsNumericString thousandSeparator="," decimalScale={2} fixedDecimalScale inputMode="decimal" allowNegative={false} placeholder="0.00" aria-label={`${entry.label} actual amount`} onValueChange={({ value }) => updateValue(`wealthActual.${entry.id}`, value)} />}
                  {!entry.autoGenerated ? <input type="text" className="build-profile-account-remark" value={profile.values[`wealthActualRemark.${entry.id}`] ?? ''} maxLength={160} placeholder="Remarks" aria-label={`${entry.label} actual remarks`} onChange={(event) => updateValue(`wealthActualRemark.${entry.id}`, event.target.value)} /> : null}
                </article>)}
              </div>}
            </details>
          })}
        ...
        <div className="build-profile-target-actions">
          ...
          <button type="button" className="loan-inline-button loan-inline-button-primary" onClick={saveActualSetup}>Save Actuals and Continue to Step 10</button>
        </div>
      </div>
    }
    ...
    if (profile.step === 10) {
      ...
      const targetRows = NET_WORTH_STATEMENT_ENTRIES
        .filter((entry) => !entry.autoGenerated && profile.values[`wealthTarget.${entry.id}`] !== undefined)
        .map((entry) => {
          const rawActual = profile.values[`wealthActual.${entry.id}`] ?? ''
          const hasActual = rawActual.trim() !== ''
          const targetAmount = Math.max(0, Number(profile.values[`wealthTarget.${entry.id}`] || 0))
      ...
      const renderEditableComparisonGroup = (
        ...
        return <section className="build-profile-net-worth-group">
          ...
          {rows.length > 0 ? rows.map((entry) => <label key={entry.id} className="build-profile-net-worth-line build-profile-comparison-input-line">
            <span>{entry.label}</span>
            {mode === 'variance' ? <strong>{entry.hasActual ? formatSignedVariance(entry.variance) : 'Pending'}</strong> : <NumericFormat
              aria-label={`${entry.label} ${mode} input`}
              aria-invalid={mode === 'target' ? entry.targetAmount <= 0 : !entry.hasActual}
              value={mode === 'target' ? profile.values[`wealthTarget.${entry.id}`] ?? '' : entry.rawActual}
              valueIsNumericString
              thousandSeparator="," decimalScale={2} fixedDecimalScale
              inputMode="decimal"
              allowNegative={false}
              placeholder="0.00"
              onValueChange={({ value }) => updateValue(`${mode === 'target' ? 'wealthTarget' : 'wealthActual'}.${entry.id}`, value)}
            />}
      ...
      const renderEditableComparisonStatement = (mode: 'target' | 'actual' | 'variance') => {
        ...
        const editorValue = mode === 'target' ? targetValue : mode === 'actual' ? (entry: typeof netWorthRows[number]) => entry.hasActual ? entry.actualAmount : 0 : (entry: typeof netWorthRows[number]) => entry.hasActual ? entry.variance : 0
        ...
        return <article className={`build-profile-comparison-statement build-profile-comparison-${mode} build-profile-comparison-input-statement`}>
          <header><span>{mode === 'target' ? 'Target' : mode === 'actual' ? 'Actual' : 'Variance'}</span><strong>{mode === 'variance' ? 'Auto-computed' : 'User Inputs'}</strong></header>
