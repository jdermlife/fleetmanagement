...
export default function BuildProfilePage() {
  ...
  const renderCurrentStep = () => {
    ...
    if (profile.step === 8) {
      ...
      const hasPositiveNetMonthlyIncome = statementGroupTotal(incomeRows) > statementGroupTotal(expenseRows)
      const renderIncomeExpenseColumn = (title: string, rows: typeof incomeExpenseRows, tone: string, requiredForCompletion = false) => <section className={`build-profile-income-expense-column build-profile-income-expense-${tone}`} aria-invalid={requiredForCompletion && !hasPositiveNetMonthlyIncome}>
        <h5>{title}</h5>
