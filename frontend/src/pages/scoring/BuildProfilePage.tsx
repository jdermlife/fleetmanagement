...
function persistProfileSnapshot(profile: ProfileData): void {
  const { completionPercent } = calculateBuildProfileCompletion(profile)
...
function calculateBuildProfileCompletion(profile: ProfileData) {
  const result = {} as Record<ProfileStep, number>
  WORKFLOW_STEPS.forEach(({ id }) => {
    if (id === 8) {
      result[id] = calculateFinancialStatementCompletion(profile.values)
    }
    else if (id === 9) {
      result[id] = calculateFinancialStatementCompletion(profile.values, 'wealthActual.')
    }
    else if (id === 10) result[id] = profile.values.wealthSetupSaved === 'true' ? 100 : 0
  ...
  return {
    ...
    completionPercent: Math.round(Object.values(result).reduce((sum, percent) => sum + percent, 0) / WORKFLOW_STEPS.length),
  }
}
...
function profileFromLoanApplication(application: LoanApplicationRecord, current: ProfileData): ProfileData {
  ...
  const hydratedValue = (structuredValue: unknown, profileKey: string): string => {
    const normalized = structuredValue === null || structuredValue === undefined
      ? ''
      : String(structuredValue)
    return normalized.trim() && normalized !== '0'
      ? normalized
      : values[profileKey] || normalized
  }
  ...
  Object.assign(values, {
    ...
    civilStatus: applicant.maritalStatus || values.civilStatus || '',
    ...
    lengthOfStay: addresses.lengthOfStay || values.lengthOfStay || '',
    homeOwnership: otherInformation.homeOwnership || values.homeOwnership || '',
    education: otherInformation.educationalAttainment || values.education || '',
    ...
    gender: applicant.gender || values.gender || '',
    dependents: String(applicant.numberOfDependents ?? ''),
    employmentHistory: hydratedValue(employment.employerBusinessName, 'employmentHistory'),
    monthlyIncome: hydratedValue(application.monthly_income || employment.grossMonthlyIncome, 'monthlyIncome'),
    otherIncome: hydratedValue(application.other_income || employment.otherSourcesOfIncome, 'otherIncome'),
    debtObligations: hydratedValue(application.debt_obligations, 'debtObligations'),
    employmentStatus: hydratedValue(employment.employmentStatus, 'employmentStatus'),
    employerName: hydratedValue(employment.employerBusinessName, 'employerName'),
    officeAddress: hydratedValue(employment.officeAddress, 'officeAddress'),
    occupation: hydratedValue(employment.occupation, 'occupation'),
    position: hydratedValue(employment.position, 'position'),
    natureOfWorkBusiness: hydratedValue(employment.natureOfWorkBusiness, 'natureOfWorkBusiness'),
    dateHired: hydratedValue(employment.dateHired, 'dateHired'),
    officePhoneNumber: hydratedValue(employment.officePhoneNumber, 'officePhoneNumber'),
    previousEmployer: hydratedValue(employment.previousEmployer, 'previousEmployer'),
    totalYearsWorking: hydratedValue(employment.totalYearsWorking, 'totalYearsWorking'),
    monthlyExpenses: hydratedValue(employment.monthlyLivingExpenses, 'monthlyExpenses'),
    otherSourcesOfIncome: hydratedValue(employment.otherSourcesOfIncome, 'otherSourcesOfIncome'),
    investmentIncome: hydratedValue(employment.investmentIncome, 'investmentIncome'),
    businessIncome: hydratedValue(employment.businessIncome, 'businessIncome'),
    pensionIncome: hydratedValue(employment.pensionIncome, 'pensionIncome'),
    employmentLocation: hydratedValue(employment.employmentLocation, 'employmentLocation'),
    employerBusinessYears: hydratedValue(employment.employerBusinessYears, 'employerBusinessYears'),
    ...
    spouseEmployerBusinessName: hydratedValue(spouse.employerBusinessName, 'spouseEmployerBusinessName'),
    spouseOfficeAddress: hydratedValue(spouse.officeAddress, 'spouseOfficeAddress'),
    spouseOccupation: hydratedValue(spouse.occupation, 'spouseOccupation'),
    spousePosition: hydratedValue(spouse.position, 'spousePosition'),
    spouseNatureOfWork: hydratedValue(spouse.natureOfWork, 'spouseNatureOfWork'),
    spousePreviousEmployer: hydratedValue(spouse.previousEmployer, 'spousePreviousEmployer'),
    spouseYearsWithEmployer: hydratedValue(spouse.yearsWithEmployer, 'spouseYearsWithEmployer'),
    spouseTotalYearsWorking: hydratedValue(spouse.totalYearsWorking, 'spouseTotalYearsWorking'),
    spouseGrossMonthlyIncome: hydratedValue(spouse.grossMonthlyIncome, 'spouseGrossMonthlyIncome'),
    spouseMonthlyExpenses: hydratedValue(spouse.monthlyExpenses, 'spouseMonthlyExpenses'),
    spouseOtherIncomeSources: hydratedValue(spouse.otherIncomeSources, 'spouseOtherIncomeSources'),
    hasCoBorrower: requirements.coBorrowers?.length
      ? 'true'
      : values.hasCoBorrower || String(profileBase.coBorrowers.length > 0),
    ...
    securityClassification: hydratedValue(collateral.securityClassification, 'securityClassification'),
    assetType: hydratedValue(collateral.assetType, 'assetType'),
    maker: hydratedValue(collateral.maker, 'maker'),
    brand: hydratedValue(collateral.brand, 'brand'),
    model: hydratedValue(collateral.model, 'model'),
    year: hydratedValue(collateral.year, 'year'),
    vehicleMarketabilityCategory: hydratedValue(collateral.vehicleMarketabilityCategory, 'vehicleMarketabilityCategory'),
    vehicleConditionCategory: hydratedValue(collateral.vehicleConditionCategory, 'vehicleConditionCategory'),
    vehicleTypeCategory: hydratedValue(collateral.vehicleTypeCategory, 'vehicleTypeCategory'),
    motorcycleIntendedUse: hydratedValue(collateral.motorcycleIntendedUse, 'motorcycleIntendedUse'),
    useAsCollateral: String(collateral.useAsCollateral),
    appraisedValue: hydratedValue(application.appraised_value, 'appraisedValue'),
    insuranceProviderCompany: hydratedValue(collateral.insuranceProviderCompany, 'insuranceProviderCompany'),
    policyNumber: hydratedValue(collateral.policyNumber, 'policyNumber'),
    orNumber: hydratedValue(collateral.orNumber, 'orNumber'),
    crNumber: hydratedValue(collateral.crNumber, 'crNumber'),
    propertyAddress: hydratedValue(property.propertyAddress, 'propertyAddress'),
    registeredOwner: hydratedValue(property.registeredOwner, 'registeredOwner'),
    lotNumber: hydratedValue(property.lotNumber, 'lotNumber'),
    blockNumber: hydratedValue(property.blockNumber, 'blockNumber'),
    tctCctNumber: hydratedValue(property.tctCctNumber, 'tctCctNumber'),
    propertyMarketabilityCategory: hydratedValue(property.propertyMarketabilityCategory, 'propertyMarketabilityCategory'),
    houseUnitModelCategory: hydratedValue(property.houseUnitModelCategory, 'houseUnitModelCategory'),
  ...
  return {
    ...
    additionalCollaterals: (collateral.additionalCollaterals?.length ? collateral.additionalCollaterals : profileBase.additionalCollaterals).map((item, index) => ({
...
function loanPayloadFromProfile(profile: ProfileData, source: LoanApplicationRecord): LoanApplicationPayload {
  ...
  Object.assign(requirements.collateralAssetDetails, {
    ...
    model: values.model || '',
    year: values.year || '',
    vehicleMarketabilityCategory: values.vehicleMarketabilityCategory || '',
...
export default function BuildProfilePage() {
  ...
  const renderCurrentStep = () => {
    ...
    if (profile.step === 8) {
      ...
      const renderIncomeExpenseColumn = (title: string, rows: typeof incomeExpenseRows, tone: string, requiredForCompletion = false) => <section className={`build-profile-income-expense-column build-profile-income-expense-${tone}`} aria-invalid={requiredForCompletion && !rows.some((entry) => profile.values[entry.id]?.trim())}>
        <h5>{title}</h5>
      ...
      return <div className="build-profile-step-content build-profile-step-eight">
        ...
        <details id="build-profile-step-8-income-expenses" className="build-profile-detail-section build-profile-net-worth-statement build-profile-income-expense-statement" open>
          ...
          <div className="build-profile-net-worth-columns build-profile-income-expense-columns">
            {renderIncomeExpenseColumn('Income', incomeRows, 'income', true)}
            {renderIncomeExpenseColumn('Expenses', expenseRows, 'expenses', true)}
        ...
        <div className="build-profile-wealth-sections">
          {STEP1_SECTION_ORDER.map((section) => {
            ...
            return <Fragment key={section}>
              <details className="build-profile-detail-section" open={wealthSectionFilter === section}>
    ...
    if (profile.step === 9) {
      ...
      const saveActualSetup = () => {
        if (calculateFinancialStatementCompletion(profile.values) < 100) {
          setSaveMessage('Complete every Step 8 current financial amount. Enter zero where none applies.')
          return
        }
        if (calculateFinancialStatementCompletion(profile.values, 'wealthActual.') < 100) {
          setSaveMessage('Complete every Step 9 desired target amount. Enter zero where none applies.')
          return
        }
        setProfile((current) => {
