...
export function estimateYearsToTargetNetWorth(
  actualNetWorth: number,
  targetNetWorth: number,
  monthlyDeclaredIncome: number,
) {
  const remainingGap = Math.max(targetNetWorth - actualNetWorth, 0);
  if (remainingGap === 0) {
    return 0;
  }

  const annualDeclaredIncome = Math.max(monthlyDeclaredIncome, 0) * 12;
  return annualDeclaredIncome > 0 ? remainingGap / annualDeclaredIncome : null;
}

export function resolveActualNetWorthPosition(
  actualEntries: Record<string, string>,
  fallbackNetWorth: number,
) {
  const balanceSheetActuals = Object.entries(actualEntries).filter(([id, value]) =>
    (id.startsWith('asset-') || id.startsWith('liability-')) && !isBlank(value),
  );

  if (balanceSheetActuals.length === 0) {
    return fallbackNetWorth;
  }

  return balanceSheetActuals.reduce((netWorth, [id, value]) => {
    const amount = toSafeNumber(value);
    return id.startsWith('asset-') ? netWorth + amount : netWorth - amount;
  }, 0);
}
...
export default function NetWorthPositioningPage() {
  ...
  const actualNetWorth = useMemo(
    () => resolveActualNetWorthPosition(actualEntries, setupNetWorth),
  ...
  const wealthCompositeScore = useMemo(() => computeWealthCompositeScore({
    netWorthPositioningScore: netWorthBuildingScore.score,
    wealthBehaviourScore,
    wealthFoundationScore: wealthFoundationScore.score,
    wealthAuthenticityScore,
  }), [netWorthBuildingScore.score, wealthAuthenticityScore, wealthBehaviourScore, wealthFoundationScore.score]);
  ...
  const persistWealthScore = useCallback(async (
    ...
    await updateLoanApplicationWealthScore(applicationNo, {
      wealth_building_score: wealthCompositeScore.score,
      wealth_grade: wealthCompositeScore.grade,
      wealth_rating: wealthCompositeScore.rating,
      wealth_component_scores: netWorthBuildingScore.componentScores,
      wealth_certification_status: certificationStatus,
    });
  }, [applicationNo, netWorthBuildingScore.componentScores, wealthCompositeScore]);
  ...
  const handleSaveDraft = useCallback(async () => {
    ...
    try {
      ...
      if (applicationNo && asOfDate) {
        const sectionTotal = (section: StatementSection) => NET_WORTH_STATEMENT_ENTRIES
          .filter((entry) => entry.section === section && !entry.autoGenerated)
          .reduce((sum, entry) => sum + toSafeNumber(amounts[entry.id]), 0);
        const totalAssets = sectionTotal('assets');
        const totalLiabilities = sectionTotal('liabilities');
        await Promise.all([
          ...
          saveLoanApplicationNetWorth(applicationNo, {
            ...
            monthly_income: sectionTotal('monthly-income'),
            monthly_expenses: sectionTotal('monthly-expenses'),
            savings_rate: netWorthBuildingScore.metrics.savingsRatePercent,
          }),
        ]);
      }
      setSetupStatusMessage(
        applicationNo && asOfDate
          ? `Draft, net worth record, and FILSCORE-Wealth result saved (${new Date(savedDraft.updatedAt).toLocaleString()}).`
          : applicationNo
            ? `Draft saved. Set an As Of date to save the net worth database record.`
  ...
  const wealthExecutiveSummaryItems = [
    {
      ...
      value: wealthCompositeScore.score.toString(),
      detail: `${wealthCompositeScore.grade} - ${wealthCompositeScore.rating}`,
    },
    {
...
<p class="footnote">Information Provided: ${completionPercent}% &nbsp; | &nbsp; AI-assisted recommendations may contain mistakes.</p></main></body></html>`, [
    ...
    wealthCompositeScore.grade,
    wealthCompositeScore.rating,
    wealthCompositeScore.score,
    ...
    <div className="psychometric-page networth-dashboard-page networth-report-page">
      ...
      <section className="psychometric-summary-grid budget-dashboard-summary-grid dashboard-five-card-summary networth-report-summary" aria-label="Net worth summary">
        ...
        <article className="psychometric-summary-card">
          ...
          <strong>{formatSignedCurrency(actualNetWorth)}</strong>
          <small>Assets less liabilities using available actual entries</small>
        </article>
        <article className="psychometric-summary-card">
          <span>Target Net Worth</span>
          <strong>{targetAmount > 0 ? formatCurrency(targetAmount) : 'Not set'}</strong>
          <small>Declared financial target</small>
        </article>
        <article className="psychometric-summary-card">
          <span>Years to Achieve Target</span>
          <strong>{yearsToTargetLabel}</strong>
          <small>Based on declared monthly income</small>
        </article>
      </section>
              ...
              <div className="loan-certification-inner">
                ...
                <div className="loan-certification-summary-grid">
                  <div className="loan-certification-summary-card networth-composite-score">
                    ...
                    <strong className="loan-certification-summary-value">{wealthCompositeScore.score}</strong>
                  </div>
                  <div className="loan-certification-summary-card">
                    <span className="loan-certification-summary-label">Label</span>
                    <strong className="loan-certification-summary-value">{wealthCompositeScore.grade}</strong>
                  </div>
                  <div className="loan-certification-summary-card">
  ...
  >
    Finish Steps 1 to 4 of the Workflow Form, then select this button to access your FILSCORE Wealth Building Scorecard and AI Driven Insights.
        ...
        <div className="psychometric-hero-metric networth-dashboard-scorecard">
          <span>Composite Wealth Score</span>
          <strong>{wealthCompositeScore.score}</strong>
          <small>{`${wealthCompositeScore.grade} - ${wealthCompositeScore.rating} | Step ${step}/${workflowSteps.length}: ${currentStepLabel}`}</small>
        </div>
            ...
            <section className="budget-workflow-ai-grid">
              <article className="psychometric-panel">
                ...
                <ul className="psychometric-breakdown-list">
                  ...
                  <li>
                    <span>Actual Net Worth Position</span>
                    <strong>{formatSignedCurrency(actualNetWorth)}</strong>
                  </li>
                  <li>
