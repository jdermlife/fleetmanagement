...
function MaintenanceManagementPage() {
  ...
  const [form, setForm] =
    useState<MaintenanceRecordSubmission>(
      initialForm,
    )
  ...
  async function loadPage() {
    ...
    try {
      ...
      setError(
        getErrorMessage(
          loadError,
          'Unable to load maintenance management data right now.',
        ),
      )
    } finally {
      setIsLoading(false)
  ...
  async function handleSubmit(
    ...
    try {
      const response =
        await api.post<MaintenanceRecord>(
          '/maintenance-records',
          form,
        )

      console.log(
        'SAVE MAINTENANCE API:',
        response.data,
      )
  ...
  function updateNumberField<
    K extends keyof MaintenanceRecordSubmission,
  >(key: K, value: string) {
    setForm((current) => ({
      ...current,
