...
describe('BuildProfilePage', () => {
  ...
  it('copies the complete Lending Scorecard banking relationships content into Step 5', async () => {
    ...
    expect(screen.getByRole('group', { name: '1. Do you have a record of unpaid debt in Credit Bureau and when?' })).toBeTruthy()
    expect(screen.getByRole('radio', { name: 'Yes - in last 5 years' })).toBeTruthy()
    expect(screen.getByRole('group', { name: '2. What is the amount of the loan?' })).toBeTruthy()
    expect(screen.getByRole('radio', { name: 'More than 1,000 but less than 5,000' })).toBeTruthy()
    expect(screen.getByRole('group', { name: '3. Is the loan paid?' })).toBeTruthy()
    expect(screen.getByRole('radio', { name: 'Fully paid with Certification' })).toBeTruthy()
    expect(screen.getByRole('radio', { name: 'Not Applicable - No loan' })).toBeTruthy()
    ...
    await user.click(screen.getByRole('radio', { name: 'Yes - in last 5 years' }))
    await user.click(screen.getByRole('radio', { name: 'More than 1,000 but less than 5,000' }))
    await user.click(screen.getByRole('radio', { name: 'Fully paid with Certification' }))
    await user.click(screen.getByRole('button', { name: 'Save Profile' }))
    ...
    expect(savedProfile.values.creditBureauUnpaidDebtRecord).toBe('Yes - in last 5 years')
    expect(savedProfile.values.creditBureauLoanAmount).toBe('More than 1,000 but less than 5,000')
    expect(savedProfile.values.creditBureauLoanPaidStatus).toBe('Fully paid with Certification')
  })
