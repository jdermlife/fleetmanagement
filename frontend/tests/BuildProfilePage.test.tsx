...
describe('BuildProfilePage', () => {
  ...
  it('copies Net Worth Positioning Step 1 into Wealth Position Base Setting', async () => {
    ...
    await user.click(screen.getByText('Net Worth', { selector: 'summary' }))
    expect(netWorthDropdown?.hasAttribute('open')).toBe(true)
    expect(within(within(netWorthDropdown!).getByText('Cash on Hand').parentElement!).getByText('₱50,000.00')).toBeTruthy()
    expect(within(within(netWorthDropdown!).getByText('Home Mortgage').parentElement!).getByText('₱20,000.00')).toBeTruthy()
    expect(within(netWorthDropdown!).getByText('+₱30,000.00')).toBeTruthy()
    await user.click(screen.getByRole('button', { name: 'Save Profile' }))
