...
describe('BuildProfilePage', () => {
  ...
  it('copies the complete Lending Scorecard banking relationships content into Step 5', async () => {
    ...
    expect(screen.getByRole('group', { name: '1. In your recollection, how many times have you had a late payment?' })).toBeTruthy()
    expect(screen.getByRole('radio', { name: 'No late payments' })).toBeTruthy()
    expect(screen.getByRole('radio', { name: '1–2 late payments (≤30 days)' })).toBeTruthy()
    expect(screen.getByRole('group', { name: '2. How many delinquencies or defaults have you had before?' })).toBeTruthy()
    expect(screen.getByRole('radio', { name: 'Current default / foreclosure / repossession' })).toBeTruthy()
    expect(screen.getByRole('group', { name: '3. What is your overall loan balance compared with your total approved credit?' })).toBeTruthy()
    ...
    expect(screen.getByRole('group', { name: '9. How many legal cases or collections do you have?' })).toBeTruthy()
